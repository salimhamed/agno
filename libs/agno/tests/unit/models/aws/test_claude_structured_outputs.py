...
(`output_config.format` + the `structured-outputs-2025-11-13` beta header) on the same
...
subclass:
  ...
  - actually emits `output_config.format` and the beta header in outgoing requests.
"""
...
class TestSupportsBedrockStructuredOutputsHelper:
    ...
    @pytest.mark.parametrize(
        "base_id",
        [
            "claude-opus-4-1-20250805",  # available on Bedrock but rejects structured outputs
